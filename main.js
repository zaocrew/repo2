// console.log("Hello world") // строка
// console.log(666) // число
// console.log(66+66)
// console.log('Hello ' + ' world')
// alert('Hello world')
// const myBirthday = '24.06.1996' неизменяемое переменная
// myBirthday = '01.01.1901'
// let myBirthday = '12.12.12'
// myBirthday = '1/1/1'
// console.log(myBirthday)

// console.log('06')
// console.log("'Добро '+'пожаловать '+' на курс' '+' 'Никита'")
// alert("17.01.2023")

//document.getElementById('first').innerHTML ='Hello World'

// 1 тип данных числовой (number)
// console.log(Infinity); //infinity
// console.log(1/0); //infinity
// console.log(NaN); // получаем NAN
// console.log('storlka' * 12); //получаем NAN

// 2 тип данных BigInt
// let a = 999999999999
// console.log(a)

// 3 тип данных строка (string)
// let str1 ="1111";
// let srt2 ="2222 $(str1)";
// let str3 = '3333 $(str1)';

// console.log(str1)
// console.log(str2)
// console.log(str3)

// 4 тип данных Логический тип данных (BOOLean)

// Let Datcheck -true
// Let DatunCheck -false
// // Let a - 4 >1
// console.log(true == 0)

// 5 тип данных null
// Let nuller - null
// console.log(nuller)

// 6 тип данных undifined

// Let test;
// console.log(test)

// 7 тип данных Обьекты

// 8 оператор typeof

// console.log(typeof undefined)
// console.log(typeof 0)
// console.log(typeof 1100n)
// console.log(typeof false)
// console.log(typeof Symbol ('id'))
// console.log(typeof null)



// let number = prompt('Число?')

// if(number > 100){
//  alert('Вывод на страницу')
//  }
//  else{
//     console.log(number)
// }


//Домашнее задание 4 - 1 часть
// let a = 10
// let b = 5
// if(a + b > 15){
//     console.log('ДА')
// }
// else if(a + b <=15{
//     console.log('НЕТ')
// }

//Домашнее задание 4 - 2 часть
// let a = 10;
// let b = '10';
//     console.log(a === b)





//Домашнее задание 5 - 1 часть(1 вариант решения)
// let i = 1;
// while(i<51){
// alert(i)
// i++}


//Домашнее задание 5 - 1 часть(2 вариант решения)
// for(let i=1; i<51;i++){
//     alert(i)
// }

//немного не понял задания, но вот вывод в консоль: 
// for(let i=1; i<51;i++){
//     console.log(i)
// }


//Домашнее задание 5 - 2 часть
// let i = 50;
// while(i>24){
// alert(i)
// i--}

//Домашнее задание 5 - 3 часть

// let i=2501;
// for(let i=2501;i<3000;i++){
//     console.log(i++)
// }
// console.log(i) 





















//УРОК 6 ДЗ

// let firstname = null;
// let lastname =  FALSE;
// let nickname = 'John Snow(Stark)'

//alert (firstname || lastname || nickname || 'Аноним')




// let number = + promt('Введите число от 0 до 3','')
// //переписать на switchcase          (else это default)
// if(number ===0){
// alert('Вы ввели число 0')
// } 
// if(number ===1){
//     alert('Вы ввели число 1')
// }
// if(number ===2 || number===3){
//     alert('Вы ввели 2 или 3')
// }
// else {
//     alert('Вы ввели некоректное значение')
// }
